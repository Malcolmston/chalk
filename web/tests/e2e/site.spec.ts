  ...
  for (const id of TAB_IDS) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (/^https?:\/\//.test(href)) {
        ...
        // The generated go/doc API reference is published alongside this site at
        // /chalk/api/ by the Pages workflow; it is not present in the preview
        // build, so we only assert the link is the expected relative target.
        expect(href, `#${id}: unexpected relative href "${href}"`).toBe('./api/');
      } else {
...
test('internal tab links actually navigate to their view', async ({ page }) => {
  await gotoTab(page, 'overview');
  // The overview "on this page" jump-nav links to the in-page Features section.
  await page.locator('.view.active a[href="#overview-feat"]').first().click();
  await expect(page.locator('.view.active')).toHaveAttribute('id', 'view-overview');
  await expect(page).toHaveURL(/#overview-feat$/);
});
